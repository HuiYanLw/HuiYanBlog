$(function(){
	$.post("/Article/GetArticleList.html", { "currentpage": 1, "numberpage": 10 },function(data){
		var pojo=eval("("+data+")");
		layui.use(['laypage', 'layer'], function(){
			  ...
			  laypage.render({
			    ...
			    ,jump: function(obj){
			    	$.post("/Article/GetArticleList.html", { "currentpage": obj.curr, "numberpage": obj.limit },function(kind){
			    		var Num=""; 
			    		for(var i=0;i<10;i++) 
			    		{ 
			    		...
			    		$("#blogsbox_alertlist").html(articlelist); 
			    	});
			    }
			  });
			});
	...
	$.post("/Article/GetLikeArticleList.html",function(data){
		var Num=""; 
		for(var i=0;i<10;i++) 
		{ 
			Num+=Math.floor(Math.random()*10); 
		...
		for (var i = 0; i < pojo.length; i++) {
			 likeArticle+='<li> <a href="/Article/GetArticleContent/'+Num+pojo[i].a_id+'.html" target="_blank">';
			 likeArticle+='<i><img src="'+pojo[i].a_img+'"></i>';
			 likeArticle+=' <h2>'+pojo[i].a_title+'</h2>';
			if(pojo[i].cl_category==1){
				 likeArticle+='<span>学无止境</span> </a> </li>';
			}
			if(pojo[i].cl_category==2){
				 likeArticle+='<span>慢生活</span> </a> </li>';
			}
		}
		$(".toppic").html(likeArticle); 
	});
});