...
//			System.out.println("加载时间 : " + a / 1000000 + "ms");
	...
	private static void init() {
		// 获取敏感词
		//System.out.println(WordFilter.class.getClassLoader().getResourceAsStream("interceptor/dd.txt"));
		addSensitiveWord(readWordFromFile("interceptor/Sensitivewords.txt"));
		addStopWord(readWordFromFile("interceptor/StopSensitivewords.txt"));//WordFilter.class.getResource("interceptor/dd.txt").toString()
	}
		...
		try {
			...
			for (String buf = ""; (buf = br.readLine()) != null;) {
				if (buf == null || buf.equals(""))
					continue;
//				buf=new String(buf.trim().getBytes("ISO-8859-1"),"UTF-8");
//				System.out.println(buf);
	 ...
	 * @return
	 */
	public static final String getBadWord(final String src) {
		String str="";
		String regEx="[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
		...
		return new String(m.replaceAll("").trim());
	}
